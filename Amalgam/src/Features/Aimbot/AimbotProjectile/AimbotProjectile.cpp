...
std::vector<Target_t> CAimbotProjectile::GetTargets(CTFPlayer* pLocal, CTFWeaponBase* pWeapon)
{
...

	{
		...
		case TF_WEAPON_LUNCHBOX: 
			if(Vars::Aimbot::Healing::AutoSandwich.Value)
				eGroupType = EntityEnum::PlayerAll; break; // all because of friends
			break;
		}
		bool bHeal = iWeaponID == TF_WEAPON_CROSSBOW || iWeaponID == TF_WEAPON_LUNCHBOX;
		...
		for (auto pEntity : H::Entities.GetGroup(eGroupType))
		{
			...
			{
				if (!pPlayer->IsAlive() || pPlayer->IsAGhost() || !bTeam && !bIsFriend)
					continue;
			}
			else if (F::AimbotGlobal.ShouldIgnore(pEntity, pLocal, pWeapon))
				continue;

			if ((bTeam || bIsFriend) && bHeal)
			{
			...
			if (!F::AimbotGlobal.PlayerBoneInFOV(pPlayer, vLocalPos, vLocalAngles, flFOVTo, vPos, vAngleTo, Vars::Aimbot::Projectile::AimFOV.Value))
				continue;
...

	{
		...
		for (auto pEntity : H::Entities.GetGroup(eGroupType))
		{
			...
			bool bTeam = pEntity->m_iTeamNum() == pLocal->m_iTeamNum();
			if (bTeam && pEntity->As<CBaseObject>()->m_iHealth() >= pEntity->As<CBaseObject>()->m_iMaxHealth() || pEntity->As<CBaseObject>()->m_bPlacing())
				continue;
...
float CAimbotProjectile::GetSplashRadius(CTFWeaponBase* pWeapon, CTFPlayer* pPlayer)
{
	if (pWeapon)
	{
		if (pWeapon->GetWeaponID() == TF_WEAPON_SPELLBOOK)
			return 200.f; // all the important spells use this
		else if (pWeapon->m_iItemDefinitionIndex() == Soldier_m_TheDirectHit)
			return 0.f; // disable on direct hit
	}
...
static inline int GetHitboxPriority(int nHitbox, Target_t& tTarget, Info_t& tInfo, CBaseEntity* pProjectile = nullptr)
{
	if (Vars::Aimbot::Projectile::Hitboxes.Value & Vars::Aimbot::Projectile::HitboxesEnum::Auto)
	{
		...
		int weaponID = tInfo.m_pWeapon->GetWeaponID();
		bool bPrioritizeFeet = Vars::Aimbot::Projectile::Hitboxes.Value & Vars::Aimbot::Projectile::HitboxesEnum::PrioritizeFeet;
		bool bValidWeapon = weaponID == TF_WEAPON_ROCKETLAUNCHER || weaponID == TF_WEAPON_ROCKETLAUNCHER_DIRECTHIT || weaponID == TF_WEAPON_PIPEBOMBLAUNCHER;

		bool bLower = bPrioritizeFeet && bValidWeapon && tTarget.m_iTargetType == TargetEnum::Player && tTarget.m_pEntity->As<CTFPlayer>()->IsOnGround();

		bool bHeadshot = Vars::Aimbot::Projectile::Hitboxes.Value & Vars::Aimbot::Projectile::HitboxesEnum::Head && tTarget.m_iTargetType == TargetEnum::Player;
		if (bHeadshot)
		{
			if (!pProjectile)
				bHeadshot = weaponID == TF_WEAPON_COMPOUND_BOW;
			else
...
std::unordered_map<int, Vec3> CAimbotProjectile::GetDirectPoints()
{
	...
	for (int i = 0; i < 3; i++)
	{
		const int iPriority = GetHitboxPriority(i, tTarget, m_tInfo, m_tInfo.m_pProjectile);
		if (iPriority == 3)
			continue;
...
bool CAimbotProjectile::HandlePoint(const Vec3& vOrigin, int iSimTime, float flPitch, float flYaw, float flTime, const Vec3& vPoint, bool bSplash)
{
	...
	else if (m_iResult != 1 && !m_tInfo.m_pProjectile)
	{
		...
		case Vars::Aimbot::General::AimTypeEnum::Smooth:
			if (Vars::Aimbot::General::AssistStrengthStart.Value == 100.f && Vars::Aimbot::General::AssistStrengthEnd.Value == 100.f)
				break;
			[[fallthrough]];
...
static inline void DrawVisuals(int iResult, Target_t& tTarget, std::vector<std::tuple<Vec3, Vec3, float>>& vPlayerPath, std::vector<Vec3>& vProjectilePath, std::vector<DrawBox_t>& vBoxes)
{
	if (G::Attacking == 1 || !Vars::Aimbot::General::AutoShoot.Value || iResult != 1)
	{
		...
		if (bPlayerPath || bProjectilePath || bBoxes || bRealPath)
		{
			...
			if (bProjectilePath)
			{
				Color_t tPathColor = Vars::Colors::ProjectilePath.Value;
				bool bRainbow = Vars::Visuals::Simulation::ProjectilePathRainbow.Value;
				if (bRainbow)
					tPathColor = { static_cast<byte>(floor(sin(I::GlobalVars->curtime) * 127.0f + 128.0f)),
						...
						Vars::Colors::ProjectilePath.Value.a };

				if (Vars::Colors::ProjectilePathIgnoreZ.Value.a)
					G::ProjectilePathStorage.emplace_back(vProjectilePath, Vars::Visuals::Simulation::Timed.Value ? -int(vProjectilePath.size()) - TIME_TO_TICKS(F::Backtrack.GetReal()) : I::GlobalVars->curtime + Vars::Visuals::Simulation::DrawDuration.Value, bRainbow ? tPathColor.Alpha(Vars::Colors::ProjectilePathIgnoreZ.Value.a) : Vars::Colors::ProjectilePathIgnoreZ.Value, Vars::Visuals::Simulation::ProjectilePath.Value);
				if (Vars::Colors::ProjectilePath.Value.a)
					G::ProjectilePathStorage.emplace_back(vProjectilePath, Vars::Visuals::Simulation::Timed.Value ? -int(vProjectilePath.size()) - TIME_TO_TICKS(F::Backtrack.GetReal()) : I::GlobalVars->curtime + Vars::Visuals::Simulation::DrawDuration.Value, tPathColor, Vars::Visuals::Simulation::ProjectilePath.Value, true);
			}
			if (bBoxes)
...
bool CAimbotProjectile::RunMain(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	for (auto& tTarget : vTargets)
	{
		...
		if (Vars::Aimbot::General::AutoShootSpells.Value)
		{
			for (int i = 0; i < MAX_WEAPONS; i++)
			{
				auto pSpellBook = pLocal->GetWeaponFromSlot(i)->As<CTFSpellBook>();
				if (!pSpellBook || pSpellBook->GetWeaponID() != TF_WEAPON_SPELLBOOK || !pSpellBook->m_iSpellCharges())
					continue;

				iResult = CanHit(tTarget, pLocal, pSpellBook);
				break;
			}
		}
		else
		...
		if (Vars::Aimbot::General::AutoShoot.Value)
		{
				...
				if (pWeapon->As<CTFGrenadeLauncher>()->m_flDetonateTime() > 0.f)
				{
					...
					{	// rerun, if we won't hit in the future, fire
						...
						if (flCharge > GRENADE_CHECK_INTERVAL)
						{
							...
							iResult2 = CanHit(tTarget2, pLocal, pWeapon);
			...
			case TF_WEAPON_CROSSBOW:
				...
				if (Vars::Aimbot::General::AutoShootSpells.Value) // bind it yourself, forces spell aimbot
				{
