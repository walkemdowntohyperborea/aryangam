...
{	// manage lagcomp
	...
	auto pTarget = pWeapon->m_hHealingTarget()->As<CTFPlayer>();
...
inline bool CAutoHeal::TraceToEntity(CTFPlayer* pPlayer, CBaseEntity* pEntity, Vec3& vPlayerOrigin, Vec3& vEntityOrigin, unsigned int nMask)
{
	CGameTrace trace = {};
...
inline float CAutoHeal::GetDamage(CBaseEntity* pProjectile, CTFPlayer* pOwner, CTFWeaponBase* pWeapon, CTFPlayer* pTarget, float flMult, Vec3 vProjectileOrigin, int* pType)
{
...
inline float CAutoHeal::GetMult(CBaseEntity* pProjectile, CTFWeaponBase* pWeapon, CTFPlayer* pTarget)
{
...
void CAutoHeal::Run(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	AutoHeal(pLocal, pMedigun, pCmd);

	ActivateOnVoice(pLocal, pMedigun, pCmd);
	m_mMedicCallers.clear();

	AutoVaccinator(pLocal, pMedigun, pCmd);
}
