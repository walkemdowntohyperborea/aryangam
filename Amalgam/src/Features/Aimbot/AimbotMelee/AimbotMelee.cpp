...
std::vector<Target_t> CAimbotMelee::GetTargets(CTFPlayer* pLocal, CTFWeaponBase* pWeapon)
{
	...
	if (Vars::Aimbot::General::Target.Value & Vars::Aimbot::General::TargetEnum::Players)
	{
		...
		auto eGroupType = !bFriendlyFire || Vars::Aimbot::General::Ignore.Value & Vars::Aimbot::General::IgnoreEnum::Team ? EntityEnum::PlayerEnemy : EntityEnum::PlayerAll;
		if (Vars::Aimbot::Melee::WhipTeam.Value &&
			!bFriendlyFire && SDK::AttribHookValue(0, "speed_buff_ally", pWeapon) > 0)
		...
		for (auto pEntity : H::Entities.GetGroup(eGroupType))
		{
			...
			if (!F::AimbotGlobal.PlayerBoneInFOV(pEntity->As<CTFPlayer>(), vLocalPos, vLocalAngles, flFOVTo, vPos, vAngleTo, Vars::Aimbot::Melee::AimFOV.Value))
				continue;

			bool bTeam = pEntity->m_iTeamNum() == pLocal->m_iTeamNum();
			int iPriority = F::AimbotGlobal.GetPriority(pEntity->entindex());
			if (bTeam && !bFriendlyFire)
				iPriority = 0;
...
{
	if (pLocal->m_iMetalCount() == 0 && pLocal->m_iClass() == TF_CLASS_ENGINEER)
		return false;
...
void CAimbotMelee::UpdateInfo(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd, std::vector<Target_t> vTargets)
{
	...
	if ((Vars::Aimbot::Melee::SwingPrediction.Value && iSimTicks || m_iDoubletapTicks) && G::CanPrimaryAttack && pWeapon->m_flSmackTime() < 0.f)
	{
		...
		{
			{
				auto& tStorage = mMoveStorage[I::EngineClient->GetLocalPlayer()];
			...
			if (i < iSimTicks - m_iDoubletapTicks)
			{
				for (auto& tTarget : vTargets)
				{
					auto& tStorage = mMoveStorage[tTarget.m_pEntity->entindex()];
		...
		for (auto& [_, tStorage] : mMoveStorage)
