...
static Vec3 GetVelocityVector(const Vec3& vForward, const Vec3& vRight, const Vec3& vUp)
{
	return (vForward + (vUp * 200.f) + (vRight * SDK::RandomFloat(-10.f, 10.f)) +
		(vUp * SDK::RandomFloat(-10.f, 10.f)));
}
...
bool CProjectileSimulation::GetInfoMain(CTFPlayer* pPlayer, CTFWeaponBase* pWeapon, Vec3 vAngles, ProjectileInfo& tProjInfo, int iFlags, float flAutoCharge)
{
	...
	if (!bInterp && G::CurrentUserCmd)
	{
		...
		case TF_WEAPON_GRAPPLINGHOOK:
		{
			...
			if (!bNoRandomAngles) // don't do angle stuff for aimbot, nospread will pick that up
				vAngles += vAngAdd;
	...
	{
		int nIndex = pWeapon->As<CTFSpellBook>()->m_iSelectedSpellIndex();

		SDK::GetProjectileFireSetup(pPlayer, vAngles, { 3.f, 7.f, -9.f }, vPos, vAngle, true, bInterp);
		...
		case 9: // bats, meteor
		{
			tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const("models/weapons/c_models/urinejar.mdl"), vPos, vAngle, { 3.f, 3.f, 3.f }, 1000.f, 1.f, 60.f };
			return true;
		}
		case 0: // fireball
			tProjInfo = { pPlayer, pWeapon, FNV1A::Hash32Const("models/weapons/w_models/w_rocket.mdl"), vPos, vAngle, { 0.f, 0.f, 0.f }, 1000.f, 0.f, 60.f };
			return true;
		default:
			return false;
		}
...
void CProjectileSimulation::GetInfo(CBaseEntity* pProjectile, ProjectileInfo& tProjInfo)
{
	...
	case ETFClassID::CTFProjectile_SpellMeteorShower:
		tProjInfo.m_bSpell = true; // only bother with spells with arcs
	}
}
...
bool CProjectileSimulation::Initialize(ProjectileInfo& tProjInfo, bool bSimulate, bool bWorld)
{
	...
	//set drag
	{
		...
		case FNV1A::Hash32Const("models/weapons/c_models/urinejar.mdl"):
			...
			{
				vDragBasis = { 0.006222f, 0.003550f, 0.005263f };
				vAngDragBasis = { 0.000778f, 0.001638f, 0.000870f };
				m_pObj->SetMass(5.f); // spells do this? idk why
			}
			break;
