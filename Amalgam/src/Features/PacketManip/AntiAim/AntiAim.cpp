...
bool CAntiAim::ShouldRun(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	if (G::Attacking == 1 || F::AutoRocketJump.IsRunning() || F::Ticks.m_bDoubletap // this m_bDoubletap check can probably be removed if we fix tickbase correctly
...
void CAntiAim::RunOverlapping(CTFPlayer* pEntity, CUserCmd* pCmd, float& flRealYaw, bool bFake, CTFWeaponBase* pWeapon, float flEpsilon)
{
	if (!Vars::AntiAim::AntiOverlap.Value || bFake)
		return;

	float flFakeYaw = GetBaseYaw(pEntity, pCmd, true) + GetYawOffset(pEntity, true, pCmd, pWeapon);
...
float CAntiAim::GetYawOffset(CTFPlayer* pEntity, bool bFake, CUserCmd* pCmd, CTFWeaponBase* pWeapon)
{
...
float CAntiAim::GetYaw(CTFPlayer* pLocal, CUserCmd* pCmd, bool bFake, CTFWeaponBase* pWeapon)
{
	float flYaw = GetBaseYaw(pLocal, pCmd, bFake) + GetYawOffset(pLocal, bFake, pCmd, pWeapon);
	RunOverlapping(pLocal, pCmd, flYaw, bFake, pWeapon);
	return flYaw;
}

float CAntiAim::GetPitch(float flCurPitch, CUserCmd* pCmd, CTFWeaponBase* pWeapon)
{
...
void CAntiAim::Run(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, CUserCmd* pCmd)
{
	...
	int iAntiBackstab = F::Misc.AntiBackstab(pLocal, pCmd, G::SendPacket);
	if (!iAntiBackstab)
	...
	vAngles.x = iAntiBackstab != 2 ? GetPitch(pCmd->viewangles.x, pCmd, pWeapon) : pCmd->viewangles.x;
	vAngles.y = !iAntiBackstab ? GetYaw(pLocal, pCmd, G::SendPacket, pWeapon) : pCmd->viewangles.y;
