...
Enum(ESP, Name = 1 << 0, Box = 1 << 1, Distance = 1 << 2, Bones = 1 << 3, HealthBar = 1 << 4, HealthText = 1 << 5, UberBar = 1 << 6, UberText = 1 << 7, ClassIcon = 1 << 8, ClassText = 1 << 9, WeaponIcon = 1 << 10, WeaponText = 1 << 11, Priority = 1 << 12, Labels = 1 << 13, Buffs = 1 << 14, Debuffs = 1 << 15, Flags = 1 << 16, LagCompensation = 1 << 17, Ping = 1 << 18, KDR = 1 << 19, Owner = 1 << 20, Level = 1 << 21, AmmoBars = 1 << 22, AmmoText = 1 << 23, IntelReturnTime = 1 << 24, Sound = 1 << 25)
...
struct Group_t
{
	...
	Color_t m_tColor = {};
	Color_t m_tOutlineColor = Color_t(0, 0, 0, 255);
	Color_t m_tHealthColorHigh = Color_t(0, 255, 100, 255);
	Color_t m_tHealthColorMid = Color_t(255, 200, 0, 255);
	Color_t m_tHealthColorLow = Color_t(255, 0, 0, 255);
	Color_t m_tHealthColorOverheal = Color_t(75, 175, 255, 255);

	bool m_bESPUseGroupColor = false;
	bool m_bTagsOverrideColor = true;
	bool m_bUseHealthGlow = false;
	...
	bool m_bOffscreenArrowsAlert = true;

	float m_flCircleSpeed = 0.5f;
