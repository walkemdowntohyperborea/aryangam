...
#include "../../Players/BanChecker/BanChecker.h"
...
void CMenu::DrawMenu()
{
	...
	if (Begin("Main", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_NoBackground))
	{
		...
		m_vWindowSize = { vWindowSize.x + H::Draw.Scale(flSideSize), vWindowSize.y };
		...
		FTabs(
			{
				...
				{ "VISUALS", "GROUPS", "MISC##", "RADAR", "MENU" },
				{ "MISC" },
		...
		if (BeginTable("AimbotTable", 2))
		{
			...
			TableNextColumn();
			{
				if (Section("General"))
				{
					...
					PushTransparent(!(FGet(Vars::Aimbot::General::Ignore) & Vars::Aimbot::General::IgnoreEnum::Invisible));
					{
						FSlider(Vars::Aimbot::General::IgnoreInvisible, FSliderEnum::Right);
					}
					PopTransparent();
					PushTransparent(Vars::Aimbot::General::AimType.Value != Vars::Aimbot::General::AimTypeEnum::Smooth &&
						Vars::Aimbot::General::AimType.Value != Vars::Aimbot::General::AimTypeEnum::Assistive);
					{
						FSlider(Vars::Aimbot::General::AssistStrengthStart, FSliderEnum::Left);
						FSlider(Vars::Aimbot::General::AssistStrengthEnd, FSliderEnum::Right);
					}
					PopTransparent();
					...
					PopTransparent();
					PushTransparent(!(FGet(Vars::Aimbot::General::Target) & Vars::Aimbot::General::TargetEnum::Stickies));
					{
						FSlider(Vars::Aimbot::General::StickyShootRange, FSliderEnum::Right);
					}
					PopTransparent();
				...
				if (Section("Backtrack", 8))
				{
					...
					FToggle(Vars::Backtrack::SmartBacktrack, FToggleEnum::Left);
					FToggle(Vars::Backtrack::BacktrackToCrosshair, FToggleEnum::Right);
				} EndSection();
				if (Vars::Debug::Options.Value)
				{
				...
				{
					FSlider(Vars::Aimbot::Healing::AimFOV);
					...
					FToggle(Vars::Aimbot::Healing::AutoSandwich, FToggleEnum::Right);
					FToggle(Vars::Aimbot::Healing::AutoVaccinator, FToggleEnum::Left);
					FToggle(Vars::Aimbot::Healing::ActivateOnVoice, FToggleEnum::Right);
					FToggle(Vars::Aimbot::Healing::AutoVaccinatorFlamethrowerDamageOnly, FToggleEnum::Left);
					FSlider(Vars::Aimbot::Healing::AutoVaccinatorBulletScale);
					FSlider(Vars::Aimbot::Healing::AutoVaccinatorBlastScale, FSliderEnum::Left);
					FSlider(Vars::Aimbot::Healing::AutoVaccinatorFireScale, FSliderEnum::Right);
				} EndSection();
			}
			/* Column 2 */
			TableNextColumn();
			{
				if (Section("Hitscan"))
				{
					...
					FSlider(Vars::Aimbot::Hitscan::AimFOV);
					FSlider(Vars::Aimbot::Hitscan::MultipointScale, FSliderEnum::Left);
					PushTransparent(!(FGet(Vars::Aimbot::Hitscan::Modifiers) & Vars::Aimbot::Hitscan::ModifiersEnum::Tapfire));
					{
						...
						FSlider(Vars::Aimbot::Hitscan::TapFireDist, FSliderEnum::Right);
					}
					PopTransparent();
				} EndSection();
				if (Vars::Debug::Options.Value)
				...
				if (Section("Projectile"))
				{
					...
					PopTransparent();
					FDropdown(Vars::Aimbot::Projectile::RocketSplashMode);
				} EndSection();
				if (Vars::Debug::Options.Value)
				{
					if (Section("##Debug Projectile"))
					{
						...
						FToggle(Vars::Aimbot::Projectile::HuntsmanPullPoint, FToggleEnum::Left);
						...
						FSlider(Vars::Aimbot::Projectile::SplashPointsArc, FSliderEnum::Left);
		...
		if (BeginTable("HvHTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Cheater Detection"))
				{
					...
					PopTransparent();
					PushTransparent(!(FGet(Vars::CheaterDetection::Methods)& Vars::CheaterDetection::MethodsEnum::CritManipulation));
					{
	...
	case 2:
	{
		if (BeginTable("DrawTable", 2))
		{
			...
			TableNextColumn();
			{
				if (Section("Line", 8))
				{
					FColorPicker(Vars::Colors::LineIgnoreZ, 0);
			...
			TableNextColumn();
			{
				if (Section("Simulation"))
				{
					...
					FToggle(Vars::Visuals::Simulation::PlayerVelocityColors, FToggleEnum::Right);
					FToggle(Vars::Visuals::Simulation::ProjectilePathRainbow, FToggleEnum::Left);
			...
			&& BeginTable("VisualsESPTable", 2))
		{
			...
			TableNextColumn();
			{
				if (Section("Color", 8))
				{
					...
					FColorPicker("Group outline color", &tGroup.m_tOutlineColor, FColorPickerEnum::Right);
					FColorPicker("Health color high", &tGroup.m_tHealthColorHigh, FColorPickerEnum::Left);
					FColorPicker("Health color mid", &tGroup.m_tHealthColorMid, FColorPickerEnum::Right);
					FColorPicker("Health color low", &tGroup.m_tHealthColorLow, FColorPickerEnum::Left);
					FColorPicker("Health color overheal", &tGroup.m_tHealthColorOverheal, FColorPickerEnum::Right);
					FColorPicker(Vars::Colors::IndicatorTextGood, FColorPickerEnum::Left);
					FColorPicker(Vars::Colors::IndicatorTextMid, FColorPickerEnum::Right);
					FColorPicker(Vars::Colors::IndicatorTextBad, FColorPickerEnum::Left);
					FColorPicker(Vars::Colors::IndicatorTextMisc, FColorPickerEnum::Right);
					FToggle("ESP uses group color", &tGroup.m_bESPUseGroupColor, FToggleEnum::Left);
			...
			TableNextColumn();
			{
				if (Section("ESP"))
				{
					...
					if (tGroup.m_iTargets & TargetsEnum::Players)
					{
						vEntries.insert(vEntries.end(), { "Uber bar", "Uber text", "Class icon", "Class text", "Weapon icon", "Weapon text", "Priority", "Labels", "Buffs", "Debuffs", "Sound" });
						vValues.insert(vValues.end(), { ESPEnum::UberBar, ESPEnum::UberText, ESPEnum::ClassIcon, ESPEnum::ClassText, ESPEnum::WeaponIcon, ESPEnum::WeaponText, ESPEnum::Priority, ESPEnum::Labels, ESPEnum::Buffs, ESPEnum::Debuffs, ESPEnum::Sound });
					}
				...
				if (Section("Glow", 8))
				{
					...
					PopTransparent();
					FToggle("Use health as color", &tGroup.m_bUseHealthGlow, FToggleEnum::Left);
					FToggle(Vars::Colors::ClassicGlow, FToggleEnum::Right);
					PushTransparent(FGet(Vars::Colors::ClassicGlow));
					{
						FToggle(Vars::Colors::FPStencil, FToggleEnum::Left);
					}
					PopTransparent();
				} EndSection();
				if (Section("Misc", 8))
				{
					...
					FToggle("Offscreen indicators", &tGroup.m_bOffscreenArrows, FToggleEnum::Left);
					SameLine(GetWindowWidth() - H::Draw.Scale(33));
					if (IconButton(ICON_MD_KEYBOARD_ARROW_DOWN))
						OpenPopup("OffscreenIndicators");
					...
					FSlider("Sound ESP Speed", &tGroup.m_flCircleSpeed, 0.f, 2.f, 0.1f, "%g", FSliderEnum::Min | FSliderEnum::Precision);
					...
					SetNextWindowSize({ H::Draw.Scale(300), 0 });
					if (FBeginPopup("OffscreenIndicators"))
					{
						...
						FToggle("Close alert", &tGroup.m_bOffscreenArrowsAlert);
		...
		if (BeginTable("VisualsMiscTable", 2))
		{
			...
			TableNextColumn();
			{
				if (Section("UI"))
				{
					...
					FToggle(Vars::Visuals::UI::ChatColors, FToggleEnum::Right);
					FToggle(Vars::Visuals::UI::CleanScreenshots, FToggleEnum::Left);
				} EndSection();
				...
				if (Section("Effects"))
				{
					...
					FSDropdown(Vars::Visuals::Effects::BulletTracer, FDropdownEnum::Left, -10);
					...
					FSlider(Vars::Visuals::Effects::RagdollForce, FSliderEnum:: Left);
					FSlider(Vars::Visuals::Effects::PyrovisionPitch, FSliderEnum::Right);
					FToggle(Vars::Visuals::Effects::DrawIconsThroughWalls, FToggleEnum::Left);
					FToggle(Vars::Visuals::Effects::DrawCritsThroughWalls, FToggleEnum::Right);
					FToggle(Vars::Visuals::Effects::DrawDamageNumbersThroughWalls, FToggleEnum::Left);
				} EndSection();
			}
			/* Column 2 */
			TableNextColumn();
			{
				...
				if (Section("World"))
				{
					FDropdown(Vars::Visuals::World::Modulations, FDropdownEnum::Left);
					FDropdown(Vars::Visuals::World::ParticleModulationStyle, FDropdownEnum::Right);
					...
					FToggle(Vars::Visuals::World::NoPropFade, FToggleEnum::Left);
					FToggle(Vars::Visuals::World::SimpleModels, FToggleEnum::Right);
					...
					{
						FColorPicker(Vars::Colors::FogModulation, FColorPickerEnum::Right);
					}
					PopTransparent();
					PushTransparent(!(FGet(Vars::Visuals::World::Modulations) & Vars::Visuals::World::ModulationsEnum::Particle));
					{
						FColorPicker(Vars::Colors::ParticleModulation, FColorPickerEnum::Left);
					}
					PopTransparent();
					PushTransparent(!(FGet(Vars::Visuals::World::Modulations) & Vars::Visuals::World::ModulationsEnum::KillstreakSheen));
					{
						FColorPicker(Vars::Colors::SheenModulation, FColorPickerEnum::Right);
					}
					PopTransparent();
				} EndSection();
			}
	...
	case 2:
	{
		if (BeginTable("VisualsRadarTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Player", 8))
				{
					FToggle(Vars::Radar::Player::Background, FToggleEnum::Left);
					FToggle(Vars::Radar::Player::Height, FToggleEnum::Right);
					FDropdown(Vars::Radar::Player::Icon);
			...
			TableNextColumn();
			{
				...
				{
					FToggle(Vars::Radar::Building::Background, FToggleEnum::Left);
					FSlider(Vars::Radar::Building::Size);
				} EndSection();
				if (Section("World", 8))
				{
					FToggle(Vars::Radar::World::Background, FToggleEnum::Left);
	...
	case 3:
	{
		if (BeginTable("MenuTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Fonts"))
				{
					FSDropdown(Vars::Menu::Fonts::ESPName, FDropdownEnum::Left);
					FDropdown(Vars::Menu::Fonts::ESPFlags, FDropdownEnum::Right | FDropdownEnum::Multi);
					FSlider(Vars::Menu::Fonts::ESPSize, FSliderEnum::Left);
					FSlider(Vars::Menu::Fonts::ESPWeight, FSliderEnum::Right);

					FSDropdown(Vars::Menu::Fonts::ESPSmallName, FDropdownEnum::Left);
					FDropdown(Vars::Menu::Fonts::ESPSmallFlags, FDropdownEnum::Right | FDropdownEnum::Multi);
					FSlider(Vars::Menu::Fonts::ESPSmallSize, FSliderEnum::Left);
					FSlider(Vars::Menu::Fonts::ESPSmallWeight, FSliderEnum::Right);

					FSDropdown(Vars::Menu::Fonts::IndicatorsName, FDropdownEnum::Left);
					FDropdown(Vars::Menu::Fonts::IndicatorsFlags, FDropdownEnum::Right | FDropdownEnum::Multi);
					FSlider(Vars::Menu::Fonts::IndicatorsSize, FSliderEnum::Left);
					FSlider(Vars::Menu::Fonts::IndicatorsWeight, FSliderEnum::Right);

					if (FButton("Apply settings"))
						H::Fonts.RemakeFonts();
				} EndSection();
			}
			/* Column 2 */
			TableNextColumn();
			{
				if (Section("Indicators"))
				{
					...
					if(Vars::Debug::Options.Value)
						FDropdown(Vars::Menu::IndicatorDisplayStyle);

					PushTransparent(!(FGet(Vars::Menu::Indicators)& Vars::Menu::IndicatorsEnum::Ticks));
					{
					...
					PushTransparent(!(Vars::Menu::TickDisplayStyle.Value == Vars::Menu::TickDisplayStyleEnum::Cathook 
						|| Vars::Menu::CritDisplayStyle.Value == Vars::Menu::CritDisplayStyleEnum::Cathook));
					{
						FSlider(Vars::Menu::CathookBarSize, FSliderEnum::Left);
						FSlider(Vars::Menu::CathookTextYPos, FSliderEnum::Right);
					}
					PopTransparent();
					PushTransparent(Vars::Menu::TickDisplayStyle.Value != Vars::Menu::TickDisplayStyleEnum::LMAOBox);
					{
						FSlider(Vars::Menu::LBoxDTBarSize, FSliderEnum::Left);
					}
					PopTransparent();
					PushTransparent(Vars::Menu::CritDisplayStyle.Value != Vars::Menu::CritDisplayStyleEnum::LMAOBox);
					{
						FSlider(Vars::Menu::LBoxCritBarSize, FSliderEnum::Right);
					}
					PopTransparent();

					FToggle(Vars::Menu::CheapText, FToggleEnum::Left);
					PushTransparent(!(FGet(Vars::Menu::Indicators) & Vars::Menu::IndicatorsEnum::Ticks));
					{
		...
		if (BeginTable("MiscTable", 2))
		{
			...
			TableNextColumn();
			{
				if (Section("Movement"))
				{
					...
					FToggle(Vars::Misc::Movement::AutoStickyJump, FToggleEnum::Right);
					...
					FToggle(Vars::Misc::Movement::CrouchSpeed, FToggleEnum::Right);
					...
					FToggle(Vars::Misc::Movement::SniperAvoidance, FToggleEnum::Right);
					FToggle(Vars::Misc::Movement::ProjectileAvoidance, FToggleEnum::Left);
					if (Vars::Debug::Options.Value)
					{
						FToggle(Vars::Misc::Movement::ProjectileAvoidanceUseWarp, FToggleEnum::Right);
						FSlider(Vars::Misc::Movement::ProjectileAvoidanceWarpDistance);
					}
				} EndSection();
				if (Vars::Debug::Options.Value)
				{
			...
			TableNextColumn();
			{
				...
				if (Section("Game", 8))
				{
					...
					FToggle(Vars::Misc::Game::AccuracyImprovements, FToggleEnum::Left);
				} EndSection();
				if (Vars::Debug::Options.Value)
				{
				...
				if (Section("Sound", 8))
				{
					...
					FToggle(Vars::Misc::Sound::GiantWeaponSounds, FToggleEnum::Left);
					if (Vars::Debug::Options.Value)
						FToggle(Vars::Misc::Sound::QuakeAnnouncer, FToggleEnum::Right);
				} EndSection();
			}
			EndTable();
		}
...
void CMenu::MenuLogs(int iTab)
{
	...
	case 0:
	{
				...
				auto drawPlayer = [&](const ListPlayer& tPlayer, int x, int y)
					{
						...
						if (!tPlayer.m_bFake)
						{
							...
							if (BeginChild(std::format("TagBar{}", tPlayer.m_iUserID).c_str(), { flWidth - lOffset - H::Draw.Scale(4), flHeight }, ImGuiWindowFlags_None, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoScrollWithMouse | ImGuiWindowFlags_NoBackground))
							{
								...
								for (auto& iID : F::PlayerUtils.m_mPlayerTags[tPlayer.m_uAccountID])
								{
								...
								if (!F::BanChecker.IsCached(tPlayer.m_uAccountID))
									F::BanChecker.QueueCheck(tPlayer.m_uAccountID);

								BanInfo_t banInfo = F::BanChecker.GetBanInfo(tPlayer.m_uAccountID);
								if (banInfo.m_bFetched || banInfo.m_bSteamAPIFetched)
								{
									if (banInfo.m_iSourceBanCount > 0)
									{
									...
									if (banInfo.m_bVACBanned && banInfo.m_iVACBanCount > 0)
									{
									...
									{
										std::string sTagName = std::format("GB: {}", banInfo.m_iGameBanCount);
										vLabels.emplace_back(sTagName, Color_t(255, 100, 200, 255));
									}

									if (banInfo.m_bCommunityBanned)
										vLabels.emplace_back("CB", Color_t(200, 100, 50, 255));
								}
						...
						if (FBeginPopup(std::format("RightClicked{}", tPlayer.m_iUserID).c_str()))
						{
							...
							BanInfo_t banInfo = F::BanChecker.GetBanInfo(tPlayer.m_uAccountID);
							if ((banInfo.m_bFetched || banInfo.m_bSteamAPIFetched) &&
								(banInfo.m_iSourceBanCount > 0 || banInfo.m_bVACBanned || banInfo.m_bGameBanned || banInfo.m_bCommunityBanned))
							{
								Divider(H::Draw.Scale(), H::Draw.Scale(1), 0);

								if (banInfo.m_iSourceBanCount > 0)
								{
									TextColored(F::Render.Inactive.Value, std::format("SourceBans: {}", banInfo.m_iSourceBanCount).c_str());

									for (auto& sDetail : banInfo.m_vBanDetails)
									{
										PushFont(F::Render.FontSmall);
										TextColored(F::Render.Inactive.Value, std::format("  {}", sDetail).c_str());
										PopFont();
									}
								}
								...
								if (banInfo.m_bVACBanned && banInfo.m_iVACBanCount > 0)
								{
									std::string sVACText = std::format("VAC Bans: {}", banInfo.m_iVACBanCount);
									if (banInfo.m_iDaysSinceLastBan > 0)
										sVACText += std::format(" ({} days ago)", banInfo.m_iDaysSinceLastBan);
									TextColored(F::Render.Inactive.Value, sVACText.c_str());
								}

								if (banInfo.m_bGameBanned && banInfo.m_iGameBanCount > 0)
								{
									std::string sGameText = std::format("Game Bans: {}", banInfo.m_iGameBanCount);
									if (banInfo.m_iDaysSinceLastBan > 0 && !banInfo.m_bVACBanned)
										sGameText += std::format(" ({} days ago)", banInfo.m_iDaysSinceLastBan);
									TextColored(F::Render.Inactive.Value, sGameText.c_str());
								}
		...
		if (Section("Tags"))
		{
			...
			auto vTable = WidgetTable(4, H::Draw.Scale(56), { GetWindowWidth() / 2, H::Draw.Scale(140), GetWindowWidth() / 2 - H::Draw.Scale(230) - GetStyle().WindowPadding.x });
			...
			if (BeginWidgetTable(1, vTable))
			{
				...
				{
					SetCursorPosY(GetCursorPos().y + H::Draw.Scale(15));
					FToggle("Killsay", &tTag.m_bKillsay, FToggleEnum::Left);
				}
				PopTransparent();
			} EndChild();
			...
			SetCursorPos(vTable[3].m_vPos);
			if (BeginChild(vTable[3].m_sName.c_str(), vTable[3].m_vSize, vTable[3].m_iWindowFlags, vTable[3].m_iChildFlags))
			{
				...
				if (bCreate)
				{
					...
					if (iID > -1 || iID < F::PlayerUtils.m_vTags.size())
					{
						...
						F::PlayerUtils.m_vTags[iID].m_bKillsay = tTag.m_bKillsay;
					}
					else
...

				{
					...
					if (FButton("Import", FButtonEnum::Fit | FButtonEnum::SameLine))
					{
						try
						{
							...
							read_json(F::Configs.m_sCorePath + "Import.json", tRead);

							mPlayerTags.clear();
							mPlayerAliases.clear();
							mAs.clear();
							vTags = {
								{ "Default", { 200, 200, 200, 255 }, 0, false, false, true, false },
								{ "Ignored", { 200, 200, 200, 255 }, -1, false, true, true, false },
								{ "Cheater", { 255, 100, 100, 255 }, 1, false, true, true, false },
								{ "Friend", { 100, 255, 100, 255 }, 0, true, false, true, false },
								{ "Party", { 100, 100, 255, 255 }, 0, true, false, true, false },
								{ "F2P", { 255, 255, 255, 255 }, 0, true, false, true, false }
							};
	...
	case 1:
	{
		if (BeginTable("ConfigSettingsTable", 2))
		{
			...
			TableNextColumn();
			{
				...
				if (Section("Deaths"))
				{
					PushTransparent(!(FGet(Vars::Logging::Logs) & Vars::Logging::LogsEnum::Deaths));
					{
						FDropdown(Vars::Logging::Deaths::LogTo);
					}
					PopTransparent();
				} EndSection();
			...
			auto drawConfigs = [](std::string& sStaticName, bool bVisual = false)
				{
					...
					for (auto& [entry, sConfigName] : vConfigs)
					{
						...
						if (FBeginPopupModal(std::format("Remove{}{}", sType, sConfigName).c_str(), nullptr, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysUseWindowPadding))
						{
...
void CMenu::DrawBinds()
{
	...
	std::function<void(int)> getBinds = [&](int iParent)
		{
			for (int iBind = 0; iBind < F::Binds.m_vBinds.size(); iBind++)
			{
				...
				{
					std::string sType; std::string sKey; std::string sState;
					switch (tBind.m_iType)
					{
					case BindEnum::Key:
					...
					case BindEnum::ItemSlot:
						sType = "Slot";
						sKey = std::format("slot {}", tBind.m_iInfo + 1);
						break;
					}
					if (tBind.m_bNot && (tBind.m_iType != BindEnum::Key || tBind.m_iInfo == BindEnum::KeyEnum::Hold))
						sKey = std::format("Not {}", sKey);

					sState = "Unknown";
					if (tBind.m_vVars.empty())
						sState = tBind.m_bActive ? "On" : "Off";
					else
					{
						auto var = tBind.m_vVars.front();
						#define IsType(t, b) b->m_iType == typeid(t).hash_code()
						#define IsCham(b) b->m_iType == typeid(std::vector<std::pair<std::string, Color_t>>).hash_code()
						if (var) // safety check
						{
							if (IsType(int, var)) // multi dropdowns and dropdowns use this along with int sliders
							{
								auto b = var->As<int>();
								sState = var->m_vValues.empty() ? std::to_string(b->Value)
									: b->m_iFlags & DROPDOWN_MULTI ? std::string("Multi-Flag") : std::string(var->m_vValues[b->Value]);
							}
							else if (IsType(float, var))
								sState = std::format("{:.3f}", var->As<float>()->Value);
							else if (IsType(bool, var))
								sState = var->As<bool>()->Value ? "On" : "Off";
							else if (IsType(std::string, var))
								sState = var->As<std::string>()->Value;
							else if (IsType(Color_t, var))
							{
								auto b = var->As<Color_t>();
								sState = std::format("{} {} {} {}", b->Value.r, b->Value.g, b->Value.b, b->Value.a);
							}
							else if (IsCham(var))
								sState = "Multi-Flag";
						}
					...
					vInfo.emplace_back(sType, tBind.m_sName.c_str(), sKey, sState, iBind, tBind);
				}
	...
	PushFont(F::Render.FontSmall);
	for (auto& [sType, sName, sKey, sState, iBind, tBind] : vInfo)
	{
	...
	if (Begin("Binds", nullptr, ImGuiWindowFlags_NoScrollbar | ImGuiWindowFlags_NoCollapse | ImGuiWindowFlags_NoTitleBar | ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoFocusOnAppearing))
	{
		...
		if (Vars::Menu::BindWindowTitle.Value)
		{
			...
			SetCursorPos({ H::Draw.Scale(7), H::Draw.Scale(7) });
			FText("Binds");
			PopFont();
		...
		if (Vars::Menu::BindWindowTitle.Value)
		{
			GetWindowDrawList()->AddLine({ vWindowPos.x + H::Draw.Scale(5), vWindowPos.y + 25 },
				{ vWindowPos.x + flWidth - H::Draw.Scale(5), vWindowPos.y + 25 },
				F::Render.Accent);
		}

		PushFont(F::Render.FontSmall);
		int i = 0; for (auto& [sType, sName, sKey, sState, iBind, tBind] : vInfo)
		{
			float flPosX = 0;
...
void CMenu::Render()
{
	...
	if (m_bIsOpen)
	{
		...
		AddDraggable("Spotify song", Vars::Menu::SpotifySongDisplay, FGet(Vars::Menu::Indicators) & Vars::Menu::IndicatorsEnum::Spotify);
