...
#include "../Visuals/ESP/WeaponNames/WeaponNames.h"
...
static std::string s_sGray = Color_t(115, 155, 155).ToHex();
...
static inline void OutputInfo(int iFlags, const char* sName, const char* sOutput, const char* sChat)
{
	int iTo = (iFlags & Vars::Logging::LogToEnum::Console ? OUTPUT_CONSOLE : 0)
		...
		| (iFlags & Vars::Logging::LogToEnum::PublicChat ? OUTPUT_PUBLIC : 0);
	if (iTo)
...
void COutput::Event(IGameEvent* pEvent, uint32_t uHash, CTFPlayer* pLocal)
{
	...
	switch (uHash)
	{
	case FNV1A::Hash32Const("vote_cast"): // Voting (f1/f2)
	{
		...
		OutputInfo(Vars::Logging::VoteCast::LogTo.Value, "Vote Cast",
			std::format("{}{} voted {}", bSameTeam ? "" : "[Other team vote] ", sName, bVotedYes ? "Yes" : "No").c_str(),
			std::format("{}{}\x1{}{}\x1 voted {}{}", s_sGray, bSameTeam ? "" : "[Other team vote] ", s_sYellow, sName, bVotedYes ? s_sRed : s_sGreen, bVotedYes ? "Yes" : "No").c_str());
	...
	case FNV1A::Hash32Const("player_changeclass"): // Class change
	{
		...
		OutputInfo(Vars::Logging::ClassChange::LogTo.Value, "Class Change", 
			std::format("{} changed class to {}", sName, SDK::GetClassByIndex(pEvent->GetInt("class"))).c_str(),
			std::format("{}{}\x1 changed class to {}", s_sYellow, sName, SDK::GetClassByIndex(pEvent->GetInt("class"))).c_str());
	...
	case FNV1A::Hash32Const("player_hurt"): // Damage
	{
		...
		OutputInfo(Vars::Logging::Damage::LogTo.Value, "Damage", 
			...
			std::format("You hit {}{}\x1 for {}{} damage{} ({} / {}{})", s_sYellow, sName, s_sRed, nDamage, s_sYellow, nHealth, iMaxHealth, bCrit ? ", crit" : bMinicrit ? ", minicrit" : "").c_str());

		return;
	}
	case FNV1A::Hash32Const("player_death"): // Deaths
	{
		auto pResource = H::Entities.GetResource();
		if (!pResource)
			return;
		...
		int iIndex = I::EngineClient->GetPlayerForUserID(pEvent->GetInt("userid"));
		F::PlayerUtils.DoKillSay(iIndex);

		if (!(Vars::Logging::Logs.Value & Vars::Logging::LogsEnum::Deaths))
			return;
		...
		auto sName = F::PlayerUtils.GetPlayerName(iIndex, pResource->GetName(iIndex));
		OutputInfo(Vars::Logging::Deaths::LogTo.Value, "Death", std::format("You killed {} with {}{}", sName, sWeapon, bCrit ? " (crit)" : bMinicrit ? " (mini crit)" : "").c_str(),
			std::format("You killed {}{}\x1 with {}{}\x1{}{}\x1", s_sYellow, sName, s_sYellow, sWeapon, bCrit ? s_sRed : s_sYellow, bCrit ? " (crit)" : bMinicrit ? " (mini crit)" : "").c_str());
	...
	case FNV1A::Hash32Const("player_connect_client"): // tags/alias (player join)
	{
		...
		try
		{
			...
			if (H::Entities.InParty(uAccountID) || H::Entities.IsFriend(uAccountID)) // ignore party and friends
				return;
...
void COutput::UserMessage(bf_read& msgData)
{
	...
	OutputInfo(Vars::Logging::VoteStart::LogTo.Value, "Vote Start", std::format("{}{} called a vote on {}", bSameTeam ? "" : "[Other team vote] ", sCallerName, sTargetName).c_str(), 
		std::format("{}{}\x1{}{}\x1 called a vote on {}{}", s_sGray, bSameTeam ? "" : "[Other team vote] ", s_sYellow, sCallerName, s_sYellow, sTargetName).c_str());
}
