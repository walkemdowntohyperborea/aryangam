...
void CPlayerlistUtils::DoKillSay(int iIndex)
{
	player_info_t pi{};
	if (!I::EngineClient->GetPlayerInfo(iIndex, &pi))
		return;

	auto it = m_mPlayerTags.find(pi.friendsID);
	if (it == m_mPlayerTags.end())
		return;

	bool bKillsay = false;
	for (int iID : it->second) {
		if (auto pTag = GetTag(iID); pTag && pTag->m_bKillsay) {
			bKillsay = true;
			break;
		}
	}
	...
	static std::ifstream file(std::filesystem::current_path().string() + "\\tf\\spam.txt");
	if (!file.is_open())
		return;
	...
	int iChoice = SDK::StdRandomInt(0, static_cast<int>(lines.size() - 1));
	std::string sSelectedLine = lines[iChoice];

	boost::replace_all(sSelectedLine, "%name%", pi.name);
	boost::replace_all(sSelectedLine, "%class%", SDK::GetClassByIndex(pEntity->As<CTFPlayer>()->m_iClass()));

	I::EngineClient->ClientCmd_Unrestricted(std::format("say \"{}\"", sSelectedLine).c_str());
}
