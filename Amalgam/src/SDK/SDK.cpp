	...
	const char* sLeft, const char* sRight)
{
	if (sLog)
	{
		...
		if (iTo & OUTPUT_PUBLIC)
			I::EngineClient->ClientCmd_Unrestricted(std::format("say \"{}\"", sLog ? sLog : sFunction).c_str());
		if (iMessageBox != -1)
...
int SDK::IsAttacking(CTFPlayer* pLocal, CTFWeaponBase* pWeapon, const CUserCmd* pCmd, bool bTickBase)
{
	...
	case TF_WEAPON_SPELLBOOK:
	{
		...
		if (iWeaponID == TF_WEAPON_SPELLBOOK)
			return true;
		...
		const int iAttack = iWeaponID == TF_WEAPON_CLEAVER ? IN_ATTACK | IN_ATTACK2 : IN_ATTACK;
	...
	case TF_WEAPON_FLAMETHROWER:
		if(!SDK::AttribHookValue(0, "set_charged_airblast", pWeapon)
			...
			: G::CanSecondaryAttack && G::LastUserCmd->buttons & IN_ATTACK2 && !(pCmd->buttons & IN_ATTACK2))
			return 1;
		break;
	case TF_WEAPON_FLAME_BALL:
		if (SDK::AttribHookValue(0, "set_charged_airblast", pWeapon))
...
void SDK::GetProjectileFireSetup(CTFPlayer* pPlayer, const Vec3& vAngIn, Vec3 vOffset, Vec3& vPosOut, Vec3& vAngOut, bool bPipes, bool bInterp, bool bAllowFlip)
{
	...
	else
	{
		...
		Math::VectorAngles(vEndPos - vPosOut);
