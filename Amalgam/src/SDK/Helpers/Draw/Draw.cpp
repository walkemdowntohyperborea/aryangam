...
void CDraw::GradientRect(int x, int y, int w, int h, Color_t tColorTop, Color_t tColorBottom, bool bHorizontal, bool bAlpha)
{
	I::MatSystemSurface->DrawSetColor(tColorTop.r, tColorTop.g, tColorTop.b, tColorTop.a);
	I::MatSystemSurface->DrawFilledRectFade(x, y, x + w, y + h, bAlpha ? tColorTop.a : 255, bAlpha ? tColorBottom.a : 255, bHorizontal);
	I::MatSystemSurface->DrawSetColor(tColorBottom.r, tColorBottom.g, tColorBottom.b, tColorBottom.a);
	I::MatSystemSurface->DrawFilledRectFade(x, y, x + w, y + h, bAlpha ? tColorTop.a : 0, bAlpha ? tColorBottom.a : 255, bHorizontal);
}
...
void CDraw::TextureSimple(int x, int y, int w, int h, int iId, EAlign eAlign)
{
	switch (eAlign)
	{
	case ALIGN_TOPLEFT: break;
...
void CDraw::RenderPath(const std::vector<std::tuple<Vec3, Vec3, float>>& vPath, Color_t tColor, bool bZBuffer, int iStyle, float flTime, int iSeparatorSpacing, float flSeparatorLength)
{
	if (!tColor.a || iStyle == Vars::Visuals::Simulation::StyleEnum::Off)
		return;

	Color_t pathColor = tColor;
	for (size_t i = 1; i < vPath.size(); i++)
	{
		if (flTime < 0.f && vPath.size() - i > -flTime)
			continue;

		const auto& [playerPath, playerVelocity, flPlayerMaxSpeed] = vPath[i];
		const auto& [playerPath2, playerVelocity2, flPlayerMaxSpeed2] = vPath[i - 1];
		if (Vars::Visuals::Simulation::PlayerVelocityColors.Value)
		{
			float flAlpha = Vars::Colors::PlayerPath.Value.a;

			constexpr float flRed = 50.f;
			float flYellow = flPlayerMaxSpeed * 0.75f;
			float flOrange = flPlayerMaxSpeed * 0.9f;
			float flBlue = flPlayerMaxSpeed * 2.25f;

			Color_t tRed = Color_t(255, 0, 0, flAlpha);
			Color_t tYellow = Color_t(255, 255, 0, flAlpha);
			Color_t tOrange = Color_t(252, 167, 30, flAlpha);
			Color_t tGreen = Color_t(0, 255, 0, flAlpha);
			Color_t tBlue = Color_t(0, 55, 255, flAlpha);

			float flPlayerVelocity = playerVelocity.Length();
			if (flPlayerVelocity <= flRed)
				pathColor = tRed;
			else if (flPlayerVelocity <= flYellow)
				pathColor = tYellow;
			else if (flPlayerVelocity <= flOrange)
				pathColor = tOrange;
			else if (flPlayerVelocity <= flBlue)
				pathColor = tGreen;
			else
				pathColor = tBlue;
		}
		...
		{
			RenderLine(playerPath2, playerPath, pathColor, bZBuffer);
			break;
		}
		case Vars::Visuals::Simulation::StyleEnum::Separators:
		{
			...
			{
				const Vec3& vStart = playerPath2;
				const Vec3& vEnd = playerPath;

				Vec3 vDir = (vEnd - vStart).Normalized2D();
				vDir = Math::RotatePoint(vDir * flSeparatorLength, {}, { 0, 90, 0 });
				RenderLine(vEnd, vEnd + vDir, pathColor, bZBuffer);
			}
			break;
		}
		...
		case Vars::Visuals::Simulation::StyleEnum::Arrows:
		{
			if (!(i % 3))
			{
				...
				if (!(vStart - vEnd).IsZero())
				{
					...
					RenderLine(vEnd, vEnd - vForward * 5 + vRight * 5, pathColor, bZBuffer);
					RenderLine(vEnd, vEnd - vForward * 5 - vRight * 5, pathColor, bZBuffer);
				}
			}
			break;
		...
		case Vars::Visuals::Simulation::StyleEnum::Boxes:
		{
			RenderLine(playerPath2, playerPath, pathColor, bZBuffer);
			if (!(i % iSeparatorSpacing))
				RenderWireframeBox(playerPath, { -1, -1, -1 }, { 1, 1, 1 }, {}, pathColor, bZBuffer);
			break;
		}
		}
	}
...
Color_t CDraw::Rainbow()
{
	return {
		byte(floor(sin(I::GlobalVars->curtime) * 127.f + 128.f)),
		byte(floor(sin(I::GlobalVars->curtime + 2.f) * 127.f + 128.f)),
		byte(floor(sin(I::GlobalVars->curtime + 4.f) * 127.f + 128.f)),
		255
	};
}