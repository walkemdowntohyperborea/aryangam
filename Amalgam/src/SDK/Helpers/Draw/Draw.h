...
public:
	...
	void GradientRect(int x, int y, int w, int h, Color_t tColorTop, Color_t tColorBottom, bool bHorizontal, bool bAlpha = false);
	...
	void TextureSimple(int x, int y, int w, int h, int iId, EAlign eAlign = ALIGN_CENTER);
	...
	void RenderPath(const std::vector<std::tuple<Vec3, Vec3, float>>& vPath, Color_t tColor, bool bZBuffer = false,
