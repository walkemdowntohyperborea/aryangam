...
/*
MAKE_HOOK(Direct3dDevice9_EndScene, U::Memory.GetVirtual(I::DirectXDevice, 42), HRESULT,
	IDirect3DDevice9* pDevice)
{
	DEBUG_RETURN(Direct3dDevice9_EndScene, pDevice);

	static void* uUndesiredRetAddr;
	void* uRetAddr = _ReturnAddress();
	if (!uUndesiredRetAddr)
	{
		MEMORY_BASIC_INFORMATION tInfo;
		VirtualQuery(uRetAddr, &tInfo, sizeof(MEMORY_BASIC_INFORMATION));

		char mod[MAX_PATH];
		GetModuleFileNameA((HMODULE)tInfo.AllocationBase, mod, MAX_PATH);

		if (strstr(mod, "gameoverlay"))
			uUndesiredRetAddr = uRetAddr;
	}

	if (uRetAddr != uUndesiredRetAddr)
