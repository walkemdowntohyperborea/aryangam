...
static inline const char* ErrorCodeToString(DWORD dwErrorCode)
{
	switch (dwErrorCode)
	{
	case DBG_RIPEXCEPTION:
		return "RIP EXCEPTION";
	...
	case STATUS_ACCESS_VIOLATION: [[likely]]
...
static LONG APIENTRY ExceptionFilter(PEXCEPTION_POINTERS ExceptionInfo)
{
	...
	const char* sError = ErrorCodeToString(ExceptionInfo->ExceptionRecord->ExceptionCode);
	if (FNV1A::Hash32(sError) == FNV1A::Hash32Const("UNKNOWN ERROR"))
		return EXCEPTION_EXECUTE_HANDLER;
